...
namespace ConstructionObjects
{
    public partial class FormDocOrderer : Form
    {
        ...
        private void SearchGrid(string search)
        {
            ...
            {
                if (!order.Deleted) table.Rows.Add(order.ID_Equipment_order_agreement, order.Sum, technics.Where(t => t.ID_Technics == order.ID_Technics).FirstOrDefault().Name, counterparties.Where(t => t.ID_Counterparty == order.ID_Counterparty).FirstOrDefault().Name);
            }
