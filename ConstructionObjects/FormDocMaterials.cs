...
namespace ConstructionObjects
{
    public partial class FormDocMaterials : Form
    {
        ...
        private void SearchGrid(string search)
        {
            ...
            {
                table.Rows.Add(order.ID_Materials_ordering_agreement, order.Sum, order.Amount, materials.Where(t => t.ID_Materials == order.ID_Materials).FirstOrDefault().Name, counterparties.Where(t => t.ID_Counterparty == order.ID_Counterparty).FirstOrDefault().Name, order.Deleted);
            }
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            if (docMaterialsGrid.SelectedRows.Count != 0)
            {
                ...
                APIHelper.DELETE("Materials_ordering_agreement", current, current.ID_Materials_ordering_agreement);
                RefreshGrid();
            }
        }
